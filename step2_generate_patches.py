...
def generate_val_sample(image_list, h5_path, patch_size):
    ...
    for idx in range(image_list.shape[0]):
        ...
        for i_patch in range(image_patches.shape[0]):
            patch_file_name = os.path.join(h5_path, 'val_sample_{0}_patch_{1}x{2}x{3}_{4}.h5'.format(i_sample, patch_size[0], patch_size[1], patch_size[2], i_patch))
...
if __name__ == "__main__": 
    ...
    for i_worker in range(num_workers):
        ...
        p = multiprocessing.Process(target=generate_random_patches, args=(train_image_list_split[i_worker],
                                                                   h5_path,
                                                                   train_patch_size,
                                                                   target_num_patches_each_label,
                                                                   valid_pct, )) 
        p.start()
        p_list.append(p)
    ...
    # training list
    train_list = glob.glob(h5_path+'sample_*.h5')
